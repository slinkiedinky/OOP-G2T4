 ...
 *
 * Responsibilities: - Generate non-overlapping appointment slots for clinics. - Create, update
 * and delete custom slots. - Enforce constraints such as doctor availability and locked slots.
 */
public class AppointmentSlotManagementService {
  ...
  @Transactional
  public List<AppointmentSlot> generateSlotsForDate(
      Long clinicId,
      LocalDate date,
      LocalTime openTime,
      LocalTime closeTime,
    ...
    try {
      ...
      List<Doctor> morningDoctors =
          allDoctors.stream()
              .filter(d -> d.getMorning() != null && d.getMorning())
              .collect(Collectors.toList());

      List<Doctor> afternoonDoctors =
          allDoctors.stream()
              .filter(d -> d.getAfternoon() != null && d.getAfternoon())
              .collect(Collectors.toList());

      log.info(
          "Found {} total doctors, {} morning available, {} afternoon available",
          allDoctors.size(),
          morningDoctors.size(),
          afternoonDoctors.size());
      ...
      var existingSlots =
          slotRepo.findByClinicIdAndStartTimeBetween(clinicId, windowStart, windowEnd);
      var lockedSlots =
      ...
      if (!lockedSlots.isEmpty()) {
        log.warn(
            "Skipping slot generation for clinic {} between {} and {}. Found {} locked slot(s) with bookings/history.",
            clinicId,
            windowStart,
            windowEnd,
            lockedSlots.size());
      ...
      if (!existingSlots.isEmpty()) {
        log.info(
            "Deleting {} existing slots between {} and {}",
            existingSlots.size(),
            windowStart,
            windowEnd);
      ...
      log.info(
          "Generating NON-OVERLAPPING slots from {} to {} with {}min duration",
          windowStart,
          windowEnd,
          slotDurationMinutes);
      ...
      while (!currentStart.plusMinutes(slotDurationMinutes).isAfter(windowEnd)) {
        ...
        if (!availableDoctors.isEmpty()) {
          ...
          log.debug(
              "Creating slot #{}: {} to {} - assigned to Dr. {}",
              ++count,
              currentStart,
              currentStart.plusMinutes(slotDurationMinutes),
              assignedDoctor.getName());
        } else {
          slot.setDoctor(null);
          log.warn(
              "Creating slot #{}: {} to {} - NO DOCTOR AVAILABLE",
              ++count,
              currentStart,
              currentStart.plusMinutes(slotDurationMinutes));
        }
      ...
      log.error(
          "ERROR in generateSlotsForDate: clinicId={}, date={}, error={}",
          clinicId,
          date,
          e.getMessage(),
          e);
  ...
  @Transactional
  public AppointmentSlot createCustomSlot(
      Long clinicId, Long doctorId, LocalDateTime startTime, LocalDateTime endTime) {
    var clinic =
        clinicRepo
            .findById(clinicId)
            .orElseThrow(
  ...
  @Transactional(readOnly = true)
  public List<AppointmentSlot> getAvailableSlotsByDoctor(
      Long doctorId, LocalDateTime startTime, LocalDateTime endTime) {
    return slotRepo.findByDoctorIdAndStartTimeBetweenAndStatus(
        doctorId, startTime, endTime, AppointmentStatus.AVAILABLE);
  }

  @Transactional(readOnly = true)
  public List<AppointmentSlot> getSlotsByClinicAndDate(
      Long clinicId, LocalDateTime startTime, LocalDateTime endTime) {
    return slotRepo.findByStartTimeBetween(startTime, endTime).stream()
  ...
  public List<String> findDatesWithSlots(Long clinicId, LocalDate startDate, LocalDate endDate) {
    log.info(
        "Finding dates with slots for clinic {} between {} and {}", clinicId, startDate, endDate);

    try {
      // Use the existing repository method to find distinct dates
      // The repository now returns strings directly
      List<String> dates =
          slotRepo.findDistinctDatesByClinicAndDateRange(
              clinicId, startDate.toString(), endDate.toString());
  ...
  @Transactional
  public void deleteSlotsByClinicAndDateRange(
      Long clinicId, LocalDateTime startTime, LocalDateTime endTime) {
    ...
    try {
      ...
      for (String dateStr : dateStrings) {
        ...
        List<AppointmentSlot> slots =
            slotRepo.findByClinicIdAndStartTimeBetween(clinicId, startOfDay, endOfDay);
        ...
        List<AppointmentSlot> deletableSlots =
            slots.stream()
        ...
        if (!lockedSlotIds.isEmpty()) {
          log.info(
              "Skipped deleting {} slot(s) on {} because they have appointment history",
              lockedSlotIds.size(),
              dateStr);
        }
      }
