...
public class NotificationService {
  ...
  public void notifyPatient(UserAccount patient, String message) {
    System.out.println(
        "Notification Sent. Recipient is: " + patient.getEmail() + ": " + message + ".");
  }
  ...
  public void notifyPatientQueue(String email, Long clinicId, int queueNum, int numberAhead) {
    ...
    message.setText(
        "Hi "
            + user.getFullname()
            + ",\nThank you for checking in at "
            + clinic.getName()
            + ".\n\n"
            + "Your current queue number is "
            + queueNum
            + ".\nThere are currently "
            + numberAhead
  ...
  @Transactional
  public void notifyFastTrackedPatient(
      String email, Long clinicId, int queueNumber, String reason) {
    var user =
        userRepo.findByEmail(email).orElseThrow(() -> new RuntimeException("User not found"));
    var clinic =
        clinicRepo.findById(clinicId).orElseThrow(() -> new RuntimeException("Clinic not found"));
    ...
    message.setText(
        "Hi "
            + user.getFullname()
            + ",\n\n"
            + "You have been fast-tracked for your appointment at "
            + clinic.getName()
            + ".\n\n"
            + "Queue number: #"
            + queueNumber
  ...
  public void notifyNextInLine(String email, Long clinicId, int qNum) {
    ...
    message.setText(
        "Hi "
            + user.getFullname()
            + ",\n\n"
            + "You are now next in line for your appointment at "
            + clinic.getName()
            + ".\n"
            + "Queue Number: #"
            + qNum
