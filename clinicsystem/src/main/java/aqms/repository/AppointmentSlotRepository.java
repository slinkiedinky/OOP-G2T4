...
public interface AppointmentSlotRepository extends JpaRepository<AppointmentSlot, Long> {
   ...
   *
   * Repository exposing queries for appointment slots including availability searches,
   * clinic/doctor-specific queries, and slot lifecycle operations.
   */
  @Query(
    """
  ...
  List<AppointmentSlot> findByDoctorIdAndStartTimeBetweenAndStatus(
      Long doctorId,
  ...
  @Modifying
  @Transactional
  void deleteByClinicIdAndStartTimeBetween(
      Long clinicId,
      LocalDateTime startTime,
      LocalDateTime endTime
  );

  List<AppointmentSlot> findByClinicIdAndStartTimeBetween(
      Long clinicId,
      LocalDateTime startTime,
      LocalDateTime endTime
  );
  ...
  @Query(
      value =
          "SELECT DISTINCT CAST(start_time AS DATE)::text FROM appointment_slot "
  ...
  @Query(
    ...
    where s.clinic.id = :clinicId
      ...
      order by s.startTime asc
    """)
  List<AppointmentSlot> findUpcomingByClinicAndDate(
      Long clinicId,
  ...
  @Query(
    ...
    order by s.startTime asc
  """)
  List<AppointmentSlot> findUpcomingByClinicDateAndDoctor(
      Long clinicId,
      LocalDateTime startOfDay,
      LocalDateTime endOfDay,
      Long doctorId);
  ...
  @Query(
      "SELECT a FROM AppointmentSlot a WHERE a.clinic.id = :clinicId "
