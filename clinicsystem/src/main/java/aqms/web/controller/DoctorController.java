...
public class DoctorController {
    ...
    public ResponseEntity<Doctor> createDoctor(@RequestBody CreateDoctorRequest request) {
        try {
            ...
            if (existingDoctors.isEmpty()) {
                ...
                long morningCount = existingDoctors.stream()
                    .filter(d -> d.getMorning() != null && d.getMorning())
                    .count();
                long afternoonCount = existingDoctors.stream()
    ...
    public ResponseEntity<Doctor> updateDoctor(@PathVariable Long id, @RequestBody UpdateDoctorRequest request) {
        ...
        boolean morning = request.morning() != null ? request.morning() : doctor.getMorning() != null ? doctor.getMorning() : true;
        boolean afternoon = request.afternoon() != null ? request.afternoon() : doctor.getAfternoon() != null ? doctor.getAfternoon() : true;
    ...
    private void validateDoctorAvailability(Long clinicId, Long doctorId, boolean morning, boolean afternoon) {
        ...
        List<Doctor> otherDoctors = allDoctors.stream()
                .filter(d -> !d.getId().equals(doctorId))
                .toList();
        
        // Count other doctors available for each session
        long morningCount = otherDoctors.stream()
                .filter(d -> d.getMorning() != null && d.getMorning())
                .count();
        
        long afternoonCount = otherDoctors.stream()
                .filter(d -> d.getAfternoon() != null && d.getAfternoon())
                .count();
