 ...
 *
 * Manages CRUD and query endpoints for doctor schedules. Returns lightweight DTOs for schedule
 * lists, and supports adding/removing availability.
 */
public class DoctorScheduleController {
  private final DoctorScheduleService scheduleService;

  @GetMapping("/doctor/{doctorId}")
  public ResponseEntity<List<DoctorScheduleDtos.ScheduleResponse>> getDoctorSchedules(
    ...
    var responses =
        schedules.stream()
            .map(
                schedule ->
                    new DoctorScheduleDtos.ScheduleResponse(
                        schedule.getId(),
  ...
  public ResponseEntity<List<DoctorScheduleDtos.ScheduleResponse>> getAvailableDoctorSchedules(
      @PathVariable Long doctorId) {
  ...
  @PostMapping("/add")
  public ResponseEntity<DoctorScheduleDtos.ScheduleResponse> addScheduleSlot(
      @RequestBody DoctorScheduleDtos.AddScheduleRequest request) {
    var schedule =
        scheduleService.addScheduleSlot(request.doctorId(), request.startTime(), request.endTime());
