 ...
 *
 * Exposes endpoints to view and modify clinic operating hours and related helper endpoints used
 * by the UI to determine if a clinic is open.
 */
...
public class ClinicOperatingHoursController {
  ...
  @GetMapping("/{clinicId}")
  public ResponseEntity<ClinicOperatingHoursDtos.OperatingHoursResponse> getOperatingHours(
      @PathVariable Long clinicId) {
    var hours = hoursService.getOperatingHours(clinicId);
    var detailedHours = hoursService.getDetailedOperatingHours(clinicId);

    var response =
        new ClinicOperatingHoursDtos.OperatingHoursResponse(
            clinicId,
  ...
  public ResponseEntity<Map<String, String>> getDetailedOperatingHours(
      @PathVariable Long clinicId) {
    var detailedHours = hoursService.getDetailedOperatingHours(clinicId);
  ...
  public ResponseEntity<Void> setOperatingHours(
      @RequestBody ClinicOperatingHoursDtos.SetOperatingHoursRequest request) {
    hoursService.setOperatingHours(request.clinicId(), request.hours());
    return ResponseEntity.ok().build();
  }

  @PutMapping("/set-daily-hours")
  public ResponseEntity<Void> setDailyHours(
      @RequestBody ClinicOperatingHoursDtos.SetDailyHoursRequest request) {
    hoursService.setDailyHours(
        request.clinicId(), request.dayOfWeek(), request.openTime(), request.closeTime());
    return ResponseEntity.ok().build();
  }

  @PutMapping("/set-closed-day")
  public ResponseEntity<Void> setClosedDay(
      @RequestBody ClinicOperatingHoursDtos.SetClosedDayRequest request) {
  ...
  public ResponseEntity<ClinicOperatingHoursDtos.IsOpenResponse> isClinicOpen(
    ...
    var response =
        new ClinicOperatingHoursDtos.IsOpenResponse(
            clinicId,
            "Clinic " + clinicId, // You might want to get actual clinic name
            dayOfWeek,
            time,
