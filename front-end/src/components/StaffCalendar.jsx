...
useEffect(() => {
  if (selectedClinic) {
    ...
    (async () => {
      ...
      // Ensure today's/day's counts are applied after month data
...
useEffect(() => {
  if (selectedClinic && selectedDate) {
    // Only load the day's data; avoid overwriting counts with a broad refresh
    // which can introduce timing mismatches for "today".
...
// - Count booked from the appointments feed (only booked entries)
// - Count available using the slots/available endpoint per day (accurate)
const eventMap = {};
const bookedIdsByDate = {};
...
data.forEach((item) => {
  ...
  if (item.patient) {
    ...
    if (!bookedIdsByDate[dateStr]) bookedIdsByDate[dateStr] = new Set();
    bookedIdsByDate[dateStr].add(item.id);
  }
});

// Fetch available slots for next 60 days and merge counts
...
for (let i = 0; i < 60; i++) {
  ...
  try {
    let slotsUrl = `/api/staff/slots/available?clinicId=${selectedClinic.id}&date=${dateStr}`;
    if (selectedDoctor) {
    ...
    const filtered = slots.filter((slot) => {
      const isNotBooked = !bookedIdsByDate[dateStr]?.has?.(slot.id);
      if (dateStr === today.toISOString().split("T")[0]) {
        return isNotBooked && new Date(slot.startTime) > now;
      }
      return isNotBooked;
    });

    // Only create/update an event if there are available slots for this day
    if (filtered.length > 0) {
      ...
      eventMap[dateStr].availableCount += filtered.length;
    }
  } catch (e) {
    console.error(`Failed to load available slots for ${dateStr}:`, e);
  }
}

const events = Object.values(eventMap)
  .filter((e) => e.bookedCount > 0 || e.availableCount > 0)
  .map((event) => ({
id: event.id,
start: event.start,
title: "",
...
console.log("Fetching booked appointments:", url);
    const res = await authFetch(url);
    const rawDayAppts = await res.json();
    const dayAppts = isToday
      ? rawDayAppts.filter(
          (entry) => entry.patient || new Date(entry.startTime) > now
        )
      : rawDayAppts;
...
// Exclude slots already booked and any that are in the past
const unbookedSlots = availableSlots.filter(
  (slot) => !dayAppts.some((appt) => appt.id === slot.id)
);
    const futureUnbookedSlots = isToday
      ? unbookedSlots.filter((slot) => new Date(slot.startTime) > now)
      : unbookedSlots;
console.log("Unbooked future slots:", futureUnbookedSlots);

      const combinedEntries = [...dayAppts, ...futureUnbookedSlots];
      setDayAppointments(combinedEntries);

      // Compute counts deterministically from the exact list the right panel uses
      // so the month tag and right panel always match.
      const bookedCount = combinedEntries.filter((e) => !!e.patient).length;
      const availableCount = combinedEntries.filter(
        (e) => !e.patient && e.status === "AVAILABLE"
      ).length;

      // Update only this date in the month event list
      setCalendarEvents((prevEvents) => {
        ...
        if (hasData) {
          map.set(normalizedDate, {
            ...
            borderColor: "transparent",
            textColor: "#0f172a",
            extendedProps: { bookedCount, availableCount },
          });
        } else {
...
{/* Available Slots */}
{dayAppointments.filter((slot) => !slot.patient && slot.status === "AVAILABLE").length >
0 && (
<>
<h4
